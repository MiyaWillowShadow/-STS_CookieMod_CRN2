...
public class PO_Beer extends AbstractPotion {
    public static final String POTION_ID = Helper.avoidDup(PO_Beer.class.getSimpleName());
    ...
    public PO_Beer() {
        super(potionStrings.NAME, POTION_ID, PotionRarity.UNCOMMON, PotionSize.SPHERE, PotionColor.ENERGY);
        this.isThrown = false;
    }
...
//        this.tips.add(new PowerTip(TipHelper.capitalize(GameDictionary.ARTIFACT.NAMES[0]), (String)GameDictionary.keywords.get(GameDictionary.ARTIFACT.NAMES[0])));
    ...
    public void use(AbstractCreature target) {
        if (AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) {
            this.addToBot(new ApplyPowerAction(P, AbstractDungeon.player, new SUP_Beer(2), 2));
        }
